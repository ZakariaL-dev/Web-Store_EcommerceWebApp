...
const DashAddingProductForm = () => {
  ...
  const [NewProduct, setNewProduct] = useState(defaultValues);
    ...
    try {
      ...
      for (const file of imageFiles) {
        ...
        const res = await fetch(
          `https://api.cloudinary.com/v1_1/${process.env.NEXT_PUBLIC_Cloud_Name}/image/upload`, // Your cloud name
          {
            method: "POST",
            body: formData,
          },
      ...
      // 2. Prepare the final product object with the new URLs
      const productToSubmit = {
        ...NewProduct,
        previewImages: uploadedImageUrls, // This pushes the links to your array
        price: Number(NewProduct.price), // Ensure price is a number
        discount: Number(NewProduct.discount),
      };

      // 3. Send to your MongoDB store
      const { success, message } = await addNewProduct(productToSubmit);
      ...
      if (success === true) {
        setImageFiles([]);
        setPreviews([]);
        setNewProduct(defaultValues);
      }

      setLoading(false);
    } catch (error) {
      HandeResults(false, `Failed to upload images to cloud: ${error}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form className="max-w-3xl mx-auto bg-gray-50 p-3 rounded-2xl border-2">
      <header className="w-full text-2xl font-bold">Add a new Product</header>
...
export default DashAddingProductForm;
